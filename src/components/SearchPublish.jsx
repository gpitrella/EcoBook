...
function SearchPublish({ book, bookList, selectedBook }) {
  const { margin, colors, normalize } = useTheme();
  const BOOKW = normalize(100, 130);
  ...
  const styles = {
    bookBox: {
      ...
      borderWidth: !selectedBook ? 1 : 2,
      borderColor: !selectedBook ? '#dbdbdb' : '#52B788',
      backgroundColor: '#fff',
      borderRadius: 10,
    },
    imgBox: {
...
export default React.memo(SearchPublish);
